...
MidiFile::MidiFile(uint8* data, long size)
{
	FBufferReader reader(data, size, false);

	char buffer[(int)HEADER_SIZE] = { 0 };
	reader.Serialize(buffer, HEADER_SIZE);

	initFromBuffer(buffer);

	for (int i = 0; i < mTrackCount; i++)
	{
		mTracks.Add(new MidiTrack(reader));
	}
...
void MidiFile::writeToBuffer(uint8* buffer)
{
	TArray<uint8> data;
	FMemoryWriter writer(data);

	writer.Serialize((char*)IDENTIFIER, 4);
	writer.Serialize(MidiUtil::intToBytes(6, 4), 4);
	writer.Serialize(MidiUtil::intToBytes(mType, 2), 2);
	writer.Serialize(MidiUtil::intToBytes(mTrackCount, 2), 2);
	writer.Serialize(MidiUtil::intToBytes(mResolution, 2), 2);
